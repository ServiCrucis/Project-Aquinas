...
void mdu_compile__handler(struct symbol** symbols, size_t position, size_t length) {
	...
	if (!errpos) {
		mdu_debug_fatalf(MDU_ALLOCATION_FAILURE, "[%s] failed to allocate memory for error string (required memory: %d bytes)", __func__, position + 1);
	}
	errpos[length - 1] = '\0';
	memset(errpos, ' ', position - 1);
	mdu_debug_fatalf(MDU_INCOMPLETE_RESULT, "[%s] unexpected symbol:\n\n\tposition:%d\n%s\n%s\n%s%s\n", __func__, position, mdu_cat_symbols(symbols, length), errpos, "|");
}
...
struct symbol* mdu_match_symbol_subset(struct node* (*pattern)(struct node* origin, struct symbol* input), struct symbol* input) {
	...
	if (!stack) {
		mdu_debug_fatalf(MDU_ALLOCATION_FAILURE, "[%s] failed to allocate memory for stack (required memory: %d bytes)", __func__, input->length * sizeof(struct symbol*));
	}
	// stack variables
	size_t offset = 0;
	size_t next = 0;
	...
	for(size_t i = 1; i < input->length; i++) {
		if (node) { // continuity sustained
			...
			if (!symbols) {
				mdu_debug_fatalf(MDU_ALLOCATION_FAILURE, "[%s] failed to allocate memory for new_symbol (required memory: %d bytes)", __func__, len * sizeof(struct symbol*));
			}
	...
	if (!stack) {
		mdu_debug_fatalf(MDU_ALLOCATION_FAILURE, "[%s] failed to shrink stack size to %d bytes from %d bytes", __func__, offset * sizeof(struct symbol*), input->length * sizeof(struct symbol*));
	}
...
struct symbol* mdu_wrap(struct symbol** symbols, size_t length, size_t dimensions) {
	...
	if (!symbol) {
		mdu_debug_fatalf(MDU_ALLOCATION_FAILURE, "[%s] failed to allocate memory for symbol contents (required memory: %d bytes)", __func__, sizeof(struct symbol));
	}
	symbol->y = symbols;
...
struct symbol* mdu_str_input(char* y, size_t length) {
	...
	if(!symbol->y) {
		mdu_debug_fatalf(MDU_ALLOCATION_FAILURE, "[%s] failed to allocate memory for symbol contents (required memory: %d bytes)", __func__, length * sizeof(struct symbol*));
	}
