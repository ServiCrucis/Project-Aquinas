...
static inline m_windows_stack_info *m_windows_stack_info_offset() {
    const machine_workeld computed_offset = sizeof(m_windows_selflike_page_list_info) + sizeof(m_windows_stack_info);
...
#undef local_LARGEST_ALLOCATION_VALUE
    ...
    if (!M_WINDOWS_PAGING_STATEHOLDS_STACK) {
        ...
        fatalf(__func__, "M_WINDOWS_PAGING_STATEHOLDS_STACK is non-NULL; unable to create stack representation of state\n");
}
